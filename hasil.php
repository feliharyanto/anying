...
$no=0;
$daftar='<td width="40">NO</td><td width="100">NO KK</td><td width="100">NIM</td><td>NAMA</td>';
...
for($i=0;$i<count($alternatif);$i++){
	...
	for($ii=0;$ii<count($kriteria);$ii++){
		$q=mysql_query("select himpunan.nama from klasifikasi inner join himpunan on klasifikasi.id_himpunan=himpunan.id_himpunan where 
			klasifikasi.id_alternatif='".$alternatif[$i][0]."' and himpunan.id_kriteria='".$kriteria[$ii][0]."'");
...
$no=0;
$daftar_1='<td width="40">NO</td><td width="100">NO KK</td><td width="100">NIM</td><td>NAMA</td>';
for($i=0;$i<count($kriteria);$i++){
...
for($i=0;$i<count($alternatif);$i++){
	...
	for($ii=0;$ii<count($kriteria);$ii++){
		$q=mysql_query("select himpunan.nilai from klasifikasi inner join himpunan on klasifikasi.id_himpunan=himpunan.id_himpunan where 
			klasifikasi.id_alternatif='".$alternatif[$i][0]."' and himpunan.id_kriteria='".$kriteria[$ii][0]."'");
		$h=mysql_fetch_array($q);
		$nilai=$h['nilai'];
...
# NORMALISASI 1
$no=0;
$daftar_2='<td width="40">NO</td><td width="100">NO KK</td><td width="100">NIM</td><td>NAMA</td>';
for($i=0;$i<count($kriteria);$i++){
...
?>
		...
		<table width="<?php echo (340+($jumlah_kriteria*180));?>" border="1" cellspacing="4" cellpadding="0" class="tabel_reg">
		  <?php echo $daftar;?>
		</table>
		</div>
		<br /><br />
		<div style="overflow:scroll;width:640px">
		<table width="<?php echo (340+($jumlah_kriteria*60));?>" border="1" cellspacing="4" cellpadding="0" class="tabel_reg">
		  <?php echo $daftar_1;?>
		</table>
		</div>
		...
		<br /><br />
		<div style="overflow:scroll;width:640px">
		<table width="100%" border="1" cellspacing="4" cellpadding="0" class="tabel_reg">
		  <?php echo $daftar_3;?>
		</table>
		</div>
		<br /><br />
